...
- generic [ref=e1]:
  - generic [ref=e2]:
    ...
    - generic [ref=e27]:
      - generic [ref=e28]:
        - paragraph [ref=e29]: Welcome u1 madaan
...
```ts
  ...
  50 |         .textContent();
  51 | 
> 52 |     expect(accountId?.trim()).toBeTruthy();
     |                               ^ Error: expect(received).toBeTruthy()
  53 | 
  54 |     console.log(
  55 |         `New Account Created: ${accountId}`
  56 |     );
  57 | 
  58 |     await this.page.screenshot({
  59 |         path: 'screenshots/account-created.png',
  60 |         fullPage: true
  61 |     });
  62 | 
  63 |     return accountId?.trim();
  64 | }
  65 | }
```