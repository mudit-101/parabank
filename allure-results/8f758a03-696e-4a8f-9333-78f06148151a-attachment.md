...
- Location: tests\ui\CreateAccount.spec.ts:6:5
...
```ts
  ...
  5  | 
  6  | test('FR-01 Create New Account', async ({ page }) => {
  7  | 
  8  |     const loginPage =
  9  |         new LoginPage(page);
  10 | 
  11 |     const openAccountPage =
  12 |         new OpenAccountPage(page);
  13 | 
  ...
  22 |         .toHaveURL(/overview.htm/);
  23 | 
  24 |     // Create Savings Account
  25 |     await openAccountPage.createNewAccount('1');
  26 | 
  27 |     const accountId =
  28 |         await openAccountPage.verifyAccountCreated();
  29 | 
  30 |     console.log(
  31 |         `Created Account ID: ${accountId}`
  32 |     );
  33 | 
> 34 |     expect(accountId).toBeTruthy();
     |                       ^ Error: expect(received).toBeTruthy()
  35 | });
```