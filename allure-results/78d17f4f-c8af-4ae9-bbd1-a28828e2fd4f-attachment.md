...
Expected substring: "Please enter a username and password."
Timeout: 5000ms
...
```ts
  ...
  4  | test('scen-9 empty login', async ({page}) => {
  5  |     const loginPage = new LoginPage(page);
  6  |     await loginPage.navigateToLoginPage();
  7  |     await loginPage.login(
  8  |         loginData.emptyUser.username,
  9  |         loginData.emptyUser.password
  10 |     );
  11 | 
> 12 |     await expect( page.locator('.error')).toContainText('Please enter a username and password.');
     |                                           ^ Error: expect(locator).toContainText(expected) failed
  13 | });
```